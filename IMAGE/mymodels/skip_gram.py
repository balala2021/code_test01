...
class PlaceImageSkipGram(nn.Module):
    ...
    def forward(self, images):
        #使用inception3 模型得到image的特征，模型输出是2048
        x1 = self.inception3(images)                            # N x 2048
        y1 = self.linear1(x1)              # N x embedding_dim                         # N
        return y1
...
#inception3 模型,返回2028
class Inception3_modified(Inception3):
    #调整后的Inception3 模型，基于cnn
    def forward(self, x):
...
#这个PlaceImageEmb 和PlaceImageSkipGram 模型没有分别吧？
class PlaceImageEmb(nn.Module):
    ...
    def forward(self, images):
        x = self.inception3(images)                            # N x 2048
        y = self.linear1(x)                                    # N x embedding_dim                    # N x embedding_dim
        #print(prod)                            # N
        return y
